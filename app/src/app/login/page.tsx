...
export default function LoginPage() {
    ...
    const handleSuccessLogin = (user: UserType, type: 'cara' | 'huella' | 'fallback') => {
        if (!user.name) {
            setCurrentUser(user);
            setAuthType(type);
            setMode('details');
        } else {
            runSuccessGreeting(user);
        }
    };
                                ...
                                <form onSubmit={handleCredentialLogin} className="space-y-4">
                                    <div>
                                        ...
                                        <div className="relative">
                                            ...
                                            <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} className="input-field pl-11" placeholder="usuario@correo.com" required />
                                        </div>
                                    </div>
                                    <div>
